...
var Swiper = function (selector, params) {
    ...
    _this.onSlideChangeEnd = function(){
        _this.callPlugins('onSlideChangeEnd');
    ...
    _this.swipeTo = function(index, speed, runCallbacks){
        _this.indexSwipeTo = parseInt(index, 10);
        _this.callPlugins('onSwipeTo', {index:_this.indexSwipeTo, speed:speed});
        if (params.loop) _this.indexSwipeTo = _this.indexSwipeTo + _this.loopedSlides;
        var currentPosition = isH ? _this.getWrapperTranslate('x') : _this.getWrapperTranslate('y');
        if (_this.indexSwipeTo > (_this.slides.length-1)) return;
        if (_this.indexSwipeTo<0) return;
        var newPosition
        if (params.slidesPerView=='auto') {
            newPosition = -_this.slidesGrid[ _this.indexSwipeTo ];
        }
        else {
            newPosition =  -_this.indexSwipeTo*slideSize;
        }
        if (newPosition < - maxWrapperPosition()) {
            newPosition = - maxWrapperPosition();
        };
        ...
        swipeToPosition(newPosition, 'to', {index:_this.indexSwipeTo, speed:speed, runCallbacks:runCallbacks});
        return true;
    }
    function swipeToPosition(newPosition, action, toOptions) {
        ...
        function anim(){
            ...
            if (condition) {
                ...
                if (_this.onSlideChangeEnd) _this.onSlideChangeEnd(_this)
        ...
        if (_this.onSlideChangeEnd) {
            if (_this.support.transitions) {
                if (params.queueEndCallbacks) {
                    ...
                    _this.wrapperTransitionEnd(_this.onSlideChangeEnd)
                }
                else _this.wrapperTransitionEnd(_this.onSlideChangeEnd)
            }
            else {
                if (!params.DOMAnimation) {
                    setTimeout(function(){
                        _this.onSlideChangeEnd(_this);
                    },10)
                }
            }
        }
